...
# for Steps 7-15, which it shares byte-for-byte with the Windows
# bootstrap, setup.ps1.
#
...
# Steps 7-15 are the opposite: fetching the project files, creating the
...
# still 15, so a log from any platform still reads "Step N/15" and stays
# directly comparable.
#
...
#
# The one thing that IS fatal is pwsh: Steps 7-15 run under it, so if
...
# once per top-level step (15 total, matching the "Step N/15" labels this
...
# This keeps the bar's step count and the actual "Step N/15" numbering
...
# "Step 3/15" etc. at each call site where it could get out of sync if
...
# --- Hand off to the shared installer for Steps 7-15 ---
...
if ! command -v pwsh >/dev/null 2>&1; then
    echo ""
    echo "ERROR: pwsh (PowerShell 7) is not available, so Steps 7-15 cannot run."
    echo ""
